...
;(function (window){
	function init (mapid) {
		...
		L.control.layers({},{
			'Soil Profile': layerSoilProfile(map. rc),
			'Lake': layerLake(map.rc),
			'Wells': layerWells(map.rc),
      'Aquifers':lauerAquifers(mao.rc),
		}).addTo(map)

		//the tile layer containing the image gerrated with gdal2tiles --leaflt
	...
	function layerBounds (map, rc, img) {
		//set marker at the image boud edges
